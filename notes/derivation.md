...
已完成同一份 EMA checkpoint 下，三種裁切方式 × 四種步數 × 三個取樣 seeds（0、42、123），共 36 組實驗。[完整結果與圖表](../README.md)；[原始數據](../results/clipping_ablation/)。

原本裁切方式在已測設定中以 20 步最低（6.369 ± 0.047）；不做中間裁切與裁切後重算噪聲都以 50 步最低（分別 4.878 ± 0.061、5.121 ± 0.052）。數值為平均 FID ± 樣本標準差，n=3。

200 步時，原本方式為 17.471 ± 0.099，重算噪聲後為 5.680 ± 0.069。這支持此模型的高步數退化對裁切處理敏感，但另外兩種方式在三個 seeds 中仍有從 50 到 200 步的小幅回升，不能將整個非單調現象完全歸因於裁切，也尚未證明高頻誤差機制。

所有方式在輸出 PNG 前仍統一裁切；no_clip 僅表示不做中間裁切。這次只更新結果與文件，現有 ddim.py 的取樣行為仍維持原樣。

後續先整理已保存的固定噪聲圖片對照，再進行獨立訓練 seeds、領域相關品質與多樣性評估、鄰近步數加密及跨資料集／schedule／eta 驗證。三個取樣 seeds 不等於三個獨立訓練模型；目前最佳步數是根據測試集 FID 探索性選出，未來須分開驗證集選擇與最終測試。
