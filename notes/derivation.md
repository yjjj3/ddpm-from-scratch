...
已完成原模型 36 組與 training seed=2026 新模型 27 組結果，共 63 組。共同步數 20、50、200 的跨模型比較使用 54 組；原模型另有 10 步的九組。每份模型分別使用取樣 seeds 0、42、123，標準差為各模型內的樣本標準差（ddof=1），不混合計算。

兩個模型的原本裁切方式都以已測的 20 步最低；不做中間裁切與裁切後重算噪聲都以 50 步最低。200 步時，原模型的原本方式／重算噪聲 FID 為 17.471／5.680，新模型為 21.481／5.419。這支持裁切處理的影響不只出現在第一個 checkpoint，但兩種替代方式在兩個模型的所有取樣 seeds 中仍從 50 到 200 步小幅回升，不能將整個非單調現象完全歸因於裁切，亦未證明高頻誤差機制。

[完整結果與圖表](../README.md)；[新增模型原始數據與訓練設定](../results/clipping_ablation/train_seed_2026/)。

新設定檔記錄 training seed=2026 與 30,000 步訓練目標；新舊權重雜湊不同。已補上 [checkpoint metadata](../results/clipping_ablation/train_seed_2026/checkpoint_metadata.json)，記錄實際完成 30,000 步、包含 EMA，且雜湊與評估結果一致。此為使用者匯出的權重檢查紀錄，不能單靠雜湊證明獨立或不中斷的訓練歷程。兩個模型的重現屬有限證據；六個取樣結果不等於六次獨立訓練。

所有方式在輸出 PNG 前仍統一裁切。原本的預覽圖片僅屬原模型，不代表新增模型。這次更新不改變 ddim.py 的取樣行為。checkpoint 關聯的實際步數紀錄已補齊；後續可進行更多獨立訓練、領域品質指標、驗證集步數選擇及跨資料集驗證。
