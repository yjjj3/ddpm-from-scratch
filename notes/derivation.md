...
新設定檔記錄 training seed=2026 與 30,000 步訓練目標；新舊權重雜湊不同。上傳 JSON 不含 checkpoint 實際完成步數，也不能單靠雜湊證明獨立訓練歷程。兩個模型的重現屬有限證據；六個取樣結果不等於六次獨立訓練。

所有方式在輸出 PNG 前仍統一裁切。原本的預覽圖片僅屬原模型，不代表新增模型。這次更新不改變 ddim.py 的取樣行為。後續可補 checkpoint 關聯的實際步數與訓練紀錄、更多獨立訓練、領域品質指標、驗證集步數選擇及跨資料集驗證。
